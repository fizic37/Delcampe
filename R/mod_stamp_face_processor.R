#' stamp_face_processor UI Function
#'
...
mod_stamp_face_processor_ui <- function(id, stamp_type = "face") {
  ns <- NS(id)
  tagList(
...
#' stamp_face_processor Server Functions
#'
#' @export
mod_stamp_face_processor_server <- function(id, stamp_type = "face", on_grid_update = NULL, on_image_upload = NULL, on_extraction_complete = NULL) {
  result <- moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$hline_moved_direct, {
      ...
      message("\n📏 H-LINE DRAG EVENT RECEIVED (stamp_face)")
    ...
    observeEvent(input$vline_moved_direct, {
      ...
      message("\n📏 V-LINE DRAG EVENT RECEIVED (stamp_face)")
      message("   Line ID: ", d$id)
      message("   Position in rendered image: ", d$pos_px_wrapper, " px")
    ...
    observeEvent({
      ...
      message("\n🔪 EXTRACTION STARTING (stamp_face)")
...
# mod_stamp_face_processor_ui("stamp_face_processor_1")

## To be copied in the server
# mod_stamp_face_processor_server("stamp_face_processor_1")
